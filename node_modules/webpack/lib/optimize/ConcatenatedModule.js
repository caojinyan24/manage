...
class ConcatenatedModule extends Module {
    ...
    identifier() {
        return this.modules.map(m = > m.identifier()
    ).
        join(" ");
    }
    ...
    source(dependencyTemplates, outputOptions, requestShortener) {
        ...
        if (allUsedNames.has(name)) {
            ...
            const symbolsInReferences = references.map(ref = > getSymbolsFromScope(ref.from, info.moduleScope)
        ).
...
class HarmonyExportImportedSpecifierDependencyConcatenatedTemplate {
    ...
    getExports(dep) {
        ...
        return importModule.providedExports.filter(exp = > exp !== "default" && !activeExports.has(exp)
    ).
        map(exp = > {
            return {
                name: exp,
